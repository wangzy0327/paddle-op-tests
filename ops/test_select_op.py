...
class TestSelectOp(OpTest):
    ...
    def build_cinn_program(self, target):
        builder = frontend.NetBuilder("select")
        ...
        tensor_data = [
            self.inputs["Condition"],
            self.inputs["X"],
            self.inputs["Y"],
        ]
        
        computation.get_tensor("Condition").from_numpy(tensor_data[0], target)
        computation.get_tensor("X").from_numpy(tensor_data[1], target)
        computation.get_tensor("Y").from_numpy(tensor_data[2], target)
        # 记录开始时间
        start_time = time.time()
        computation.execute()
        end_time = time.time()
